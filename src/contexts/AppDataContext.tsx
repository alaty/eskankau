...
import type { BuildingData, NewBuildingData, YearlyForecastInput, Unit, MaintenanceRecord, PaymentPlan, PaymentInstallment, ClaimRecord } from '@/lib/types';
...
interface AppContextType {
  ...
  addUnit: (buildingId: number, unitType: 'apartments' | 'suites', floor: number, newData: { unitNumber: number, baseRent: number }) => void;
...
export const AppDataProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    const generateUnits = (buildingId: number, unitType: 'apartment' | 'suite', floorConfig: { floor: number, count: number }[], baseRent: number): Unit[] => {
        ...
        floorConfig.forEach(({ floor, count }) => {
            for (let i = 0; i < count; i++) {
                units.push({
                    id: `B${buildingId}-F${floor}-${unitType.charAt(0).toUpperCase()}${unitCounter.toString().padStart(3, '0')}-${Date.now() + i}`,
  ...
  const addUnit = (buildingId: number, unitType: 'apartments' | 'suites', floor: number, newData: { unitNumber: number, baseRent: number }) => {
    const newBuildings = buildings.map(b => {
        if (b.id === buildingId) {
            const newUnit: Unit = {
          ...
          if (u.id === unitId) {
            ...
            if (plan) {
              ...
              if ((plan.type === 'installment' || plan.type === 'stipend') && areAllInstallmentsPaid) {
                  ...
                  // Ensure paymentPlan is set to correctly identify it for lost revenue
                  updatedUnit.paymentPlan = { type: plan.type }; 
  ...
  const updateAllBaseRents = (apartmentRent: number, suiteRent: number) => {
    const updatedBuildings = buildings.map(b => {
        ...
        return {
            ...b,
            apartments: { ...b.apartments, rent: apartmentRent, units: updatedAptUnits },
            suites: { ...b.suites, rent: suiteRent, units: updatedSuiteUnits }
        };
    });
